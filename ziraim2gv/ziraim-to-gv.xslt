...
<xsl:value-of select="$gvid"/> [shape=rect fontcolor=black fillcolor=salmon label="<xsl:value-of select="name"/>" tooltip="<xsl:value-of select="$tooltip"/>"];
				</xsl:when>
				<xsl:when test="exists(//relationships/relationship[destId=$id and type='Aggregation']) or
					tag[@name='ZiRA:isAggregatie']/@value='1'">
<xsl:value-of select="$gvid"/> [shape=rect fontcolor=black fillcolor=green label="<xsl:value-of select="name"/>" tooltip="<xsl:value-of select="$tooltip"/>"];
				</xsl:when>
				<xsl:when test="@isAbstract='true'">
<xsl:value-of select="$gvid"/> [shape=rect fontcolor=white fillcolor="#ffff99" label="<xsl:value-of select="name"/>" tooltip="<xsl:value-of select="$tooltip"/>"];
				</xsl:when>
				<xsl:otherwise>
<xsl:value-of select="$gvid"/> [shape=rect fillcolor="#ffff99" label="<xsl:value-of select="name"/>" tooltip="<xsl:value-of select="$tooltip"/>"];
				</xsl:otherwise>
			</xsl:choose>
		</xsl:if>
	</xsl:template>
